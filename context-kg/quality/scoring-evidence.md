...
横向比较里，路由器要对照三条标准基线（LLMRouterBench / RouterBench / Google Universal Model Routing 的通行口径），
候选池只含固定模型——它们才是路由器的备选，Fusion 变体不能充当自己的基线。BestSingle 是该赛道准确率最高的固定
模型（并列时取每题成本更低的那个），代表不做路由、全程只用一个模型；Oracle 是逐题挑出答对且每题成本最低的固定
模型，是任何路由器的理论上界，配套给出可路由题占比（最便宜的固定模型答错、但别的固定模型答对）与无人答对占比，
可路由题越少，该题集越看不出路由能力；Random 是逐题在固定模型里均匀随机挑一个，期望准确率与成本都是池内均值。
三者都在同一批冻结题 id 上计算，只有分数为 1 才算答对，出错与无分一律算答错，分母仍是计划题数。固定模型在
（每题成本，准确率）平面上的帕累托前沿连线即「随机混合线」，是输入无关路由能达到的上界；路由器落在该线左上方
才说明它在按题选模型，落在线上或右下方则说明没有。
